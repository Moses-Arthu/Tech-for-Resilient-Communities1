...
import { getGMetAWSData, getRainfallNowcast } from '../services/api';
import { CloudRain, Compass, ShieldAlert, Thermometer, Droplet, ArrowUpRight } from 'lucide-react';

export default function FloodPrediction() {
  const [gmetData, setGmetData] = useState(null);
  const [rainsatData, setRainsatData] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const rainsat = await getRainfallNowcast(5.6037, -0.1870); // Accra lat/lon
      setGmetData(gmet);
      setRainsatData(rainsat);
      setLoading(false);
    };
    fetchData();
  }, []);
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-4 shadow-sm">
          <div className="p-3 rounded-lg bg-indigo-50 text-indigo-600"><CloudRain size={24} /></div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
          <div>
            <h3 className="text-base font-bold text-slate-800">Rainsat Realtime Nowcasting (Next 5 Hours)</h3>
            <p className="text-xs text-slate-400">Lagrangian extrapolation showing upcoming precipitation probabilities.</p>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={rainsatData?.data?.nowcast} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="time" tick={{ fill: '#64748b', fontSize: 11, fontWeight: 'bold' }} />
                <YAxis tick={{ fill: '#64748b', fontSize: 11 }} />
                <Tooltip contentStyle={{ fontSize: '11px', fontFamily: 'monospace' }} />
                <Bar dataKey="probability" name="Probability (%)" fill="#3B82F6" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="flex justify-between items-center text-xs border-t pt-3 font-semibold text-slate-500">
            <span>Rainsat Status: {rainsatData?.status}</span>
            <span>Study Model Calibration: White Volta Basin Gauge</span>
          </div>
        </div>
      </div>

      {/* Threshold Matrix & Interactive Calculator */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Risk Thresholds Matrix */}
