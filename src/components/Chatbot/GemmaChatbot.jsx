      ...
      <div className="border-b bg-white dark:bg-gray-800 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Gemma AI Travel Safety Assistant</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Powered by Live Data & Google Gemma AI</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <span className={`text-xs px-3 py-1 rounded-full flex items-center ${apiMode === 'groq' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' : 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'}`}>
              <span className={`w-2 h-2 rounded-full mr-2 animate-pulse ${apiMode === 'groq' ? 'bg-blue-500' : 'bg-green-500'}`}></span>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg border border-gray-200 dark:border-gray-600">
                ...
                <div className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm dark:text-white"
                  />
                  <button onClick={() => saveApiKey(apiKey)} className="bg-blue-600 text-white px-4 py-1.5 rounded text-sm hover:bg-blue-700">Save</button>
                </div>
              </div>
            )}
          </div>
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800">
        <div className="flex items-center space-x-2">
          <input
            ...
            placeholder="Ask about live travel safety (e.g. Accra, Kumasi, Takoradi)..."
            className="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 px-4 py-3 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all shadow-sm"
            disabled={loading || (apiMode === 'groq' && !apiKey)}
          />
          <button
            type="submit"
            disabled={loading || !input.trim() || (apiMode === 'groq' && !apiKey)}
            className="rounded-lg bg-blue-600 px-6 py-3 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold shadow-sm flex items-center space-x-2"
          >
